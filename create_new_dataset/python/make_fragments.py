...
def process_fragment_olc(s,sid,eid,fragment_id, n_fragments,color_files, depth_files,intrinsic, config,pose_graph):
    for t in range(s + 1, eid):
        # odometry
        if t == s + 1: # align successive frames only
            print(
                "Fragment %03d / %03d :: RGBD matching between frame : %d and %d"
                % (fragment_id, n_fragments - 1, s, t))
            [success, trans,
             info] = register_one_rgbd_pair(s, t, color_files, depth_files,
                                            intrinsic, with_opencv, config) # find transform from one frame to the successive one
            trans_odometry = np.dot(trans, trans_odometry) # multiply current by previous transform
            trans_odometry_inv = np.linalg.inv(trans_odometry) # invert odometry
...
def register_one_rgbd_pair(s, t, color_files, depth_files, intrinsic,
    ...
    if abs(s - t) is not 1:
        ...
        odo_init = np.identity(4)  # starting point to compute the rgbd odometry
        [success, trans, info] = o3d.pipelines.odometry.compute_rgbd_odometry(
            source_rgbd_image, target_rgbd_image, intrinsic, odo_init,
            o3d.pipelines.odometry.RGBDOdometryJacobianFromHybridTerm(), option)
        return [success, trans, info]
            ...
            if t == s + 1:  # align successive frames only
                ...
                [success, trans,
                 info] = register_one_rgbd_pair(s, t, color_files, depth_files,
                                                ...
                                                config)  # find transform from one frame to the successive one
                trans_odometry = np.dot(trans, trans_odometry)  # multiply current by previous transform
                trans_odometry_inv = np.linalg.inv(trans_odometry)  # invert odometry
                pose_graph.nodes.append(
                    o3d.pipelines.registration.PoseGraphNode(trans_odometry_inv))
